        ...
        if ctx.author.name in ("enigma03c", "dragonrage9911956"):
            if channel:
                try:
                    ...
                    def process_whisper(
                        recognizer: sr.Recognizer.adjust_for_ambient_noise,
                        audio: sr.AudioData,
                        user,
                    ):
                        ...
                        try:
                            func = getattr(
                                recognizer,
                                "recognize_" + self.default_recognizer,
                                recognizer.recognize_whisper,
                            )
                            text = (
                                str(func(audio, language="english", translate=False))
                                .lower()
                                .strip()
                            )
                    ...
                    voice_client.listen(
                        SpeechRecognitionSink(
                            process_cb=process_whisper,
                            default_recognizer=self.whisper_model,
                            ignore_silence_packets=False,
                            phrase_time_limit=30,
                        )
                    )
                    await ctx.channel.send(
                        f"Got it, moving to voice channel {ctx.author.voice.channel.name} and directing output to {ctx.author.voice.channel}."
                    )
                except Exception as e:
                    await ctx.channel.send(
                        f"Failed to connect to the voice channel: {str(e)}"
                    )
            else:
                await ctx.send(
                    "Couldn't join the channel, someone tell enigma to enter first"
                )
                return
        else:
